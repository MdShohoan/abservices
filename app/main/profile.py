...
@profile.route('/home/<requestid>/', methods=['GET', 'POST'])
def home(requestid):
	error = None
	...
	if response:
		customer.status = 1
		customer.email_verify_status = 1
		#print (customer)
		#print ('status 1 update  ')
		db.session.commit() 
	...
	return render_template('profile/success.html')
...
def send_otp (account_no,customer_data = {} ) :
	...
	try:
		...
		if ssl_otp_response.get('status') == 'SUCCESS' or ssl_otp_response.get('status') == 'success':
...
def update_email_cbs(requestid,email,custMnem):
	...
	try:
		...
		api_response=app.utils.addCustomerDetails(custMnem,custLoc,email)
	except Exception as e:
		print (e)
...
def  do_account_verifiy (requestid, account_no,csrf='') :
	...
	try:
		...
		if account_no:
			...
			accountStatus = app.utils.customerEquationDetail(account_no)
			...
			#print (" customerEquationDetail :")
			
			eqresult = app.utils.customerEquationDetail(account_no)
			#print (eqresult)
			...
			cusresult = app.utils.customerDetails(custno)
...
@profile.route('/email/<requestid>/<request_type>', methods=['GET', 'POST'])
...
def email(requestid,request_type):
	...
	if request.method == 'POST':
		...
		if item :
			...
			msg                  = 'DO NOT DISCLOSE YOUR OTP TO ANYONE. An OTP has been sent to your Email. Please verify. For any Query call 16207'
...
def index():
	...
	if request.method == 'POST':
		...
		if customer_found : 
			...
			if (len(mobile)>13):
				flash ('You mobile information is not correct. To update mobile, please contact with Branch. ')
			...
			flash ('Sorry! Your email is already registered. Please contact your branch to update your email address. ')
		return render_template('profile/index.html')
...
@profile.route("/verify/<code>/<type>/",methods = ['GET', 'POST'])
...
@profile.route("/cancel/",methods = ['GET', 'POST'])
def otp_cancel():
	if request.method == "POST":
		try:
			csrf = request.form['csrf_token']
...
@profile.route("/resend/",methods = ['POST'])
def otp_resend():
	...
	if request.method == "POST":
		...
		try:
			...
			if db_result_item:
				#print ('db result ... test ')
				otp_limit_status,url = app.utils.checkOTPLimitExceed(trackingcode,current_app.config['MAX_OTP_LIMIT'],db_result_item.otp_request_count)
			...
			#DO NOT DISCLOSE YOUR OTP TO ANYONE. <OTPCode> is your OTP to download Statement at abbl.com This OTP will expire in 3 minutes. For any query call 16207
			#otp_msg = ''
			#otp_msg                  = 'DO NOT DISCLOSE YOUR OTP TO ANYONE. '+otp_code+ ' is your OTP to download Statement at es.abbl.com This OTP will expire in 3 minutes. For any Query call 16207'
			otp_msg                  	= 'DO NOT DISCLOSE YOUR OTP TO ANYONE. Your OTP is '+otp_code+ ' to update your email This OTP will expire in 3 minutes. For any Query call 16207'
			now                       = datetime.datetime.now()
			otp_send_date             = now.strftime("%Y-%m-%d %H:%M:%S")
			csmsid                    = random.randint(1, 99999999)
...
@profile.route("/emailresend/",methods = ['POST'])
def otp_eresend():
